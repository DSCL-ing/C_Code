...
可变参数至少需要固定一个形参,否则会报错,为什么? 因为可变参数部分参数类型是不确定的,每次传参传什么类型也是不确定的,要,
...
在C语言中，可变参数的机制是通过`stdarg.h`头文件中的宏和函数来实现的，例如`va_start`、`va_arg`和`va_end`等。这些宏和函数需要至少一个明确的参数，以确定可变参数的起始位置和类型。
